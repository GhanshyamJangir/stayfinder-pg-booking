...
export async function POST(req){
 try{
  ...
  const form=await req.formData();const file=form.get('file'),docType=String(form.get('docType')||'').trim();
  ...
  if(existing.some(x=>x.doc_type===docType&&x.status!=='Rejected'))throw new Error(`${docType} is already submitted.`);
