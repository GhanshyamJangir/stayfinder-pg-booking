...
async function detectFaces(canvas){
 ...
 const found=await api.detectAllFaces(canvas,new api.TinyFaceDetectorOptions({inputSize:224,scoreThreshold:.5}));
 return found.length;
}
...
export default function VerificationSetup({mode='all',items:externalItems,onItemsChange,title=true}){
 ...
 async function captureSelfie(){
  ...
  }catch(e){setFaceStatus('');setError(e.message||'Face verification failed.');}finally{setBusy(false);}
 }
