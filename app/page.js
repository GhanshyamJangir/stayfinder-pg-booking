...
export default function LoginPage() {
  ...
  useEffect(() => {
    fetch('/api/auth/me', { credentials: 'include', cache: 'no-store' }).then(async r => {
      ...
      if (data.user?.role === 'customer') { const pg=new URLSearchParams(window.location.search).get('pg'); router.replace(pg?`/customer?pg=${encodeURIComponent(pg)}`:'/customer'); }
  ...
  async function login(e) {
    ...
    try {
      ...
      const target = data.user.role === 'admin' ? '/admin' : data.user.role === 'owner' ? '/owner' : (pg?`/customer?pg=${encodeURIComponent(pg)}`:'/customer');
      ...
      {showInstallHelp && (
        <div className="installHelpBackdrop" role="dialog" aria-modal="true" aria-label="Install StayFinder" onClick={() => setShowInstallHelp(false)}>
          <div className="installHelpCard" onClick={e => e.stopPropagation()}>
            ...
            <div className="installHelpIcon"><img src="/stayfinder-icon.svg" alt="StayFinder" style={{width:'100%',height:'100%',objectFit:'contain'}}/></div>
