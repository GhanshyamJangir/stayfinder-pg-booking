...
export async function requestCheckinOtp(booking,customerId){
  ...
  if(rec?.checkin_verified_at||rec?.status==='Checked In')throw new Error('This booking is already checked in.');
...
export async function requestCheckinQr(booking){
  ...
  if(rec?.checkin_verified_at||rec?.status==='Checked In')throw new Error('This booking is already checked in. QR cannot be generated again.');
...
export async function verifyCheckinQr(bookingId,token){
  ...
  if(rec?.checkin_verified_at||rec?.status==='Checked In')throw new Error('This QR has already been used for check-in.');
