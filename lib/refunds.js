...
import { releaseInventoryLock } from './platform-controls';
...
export async function initiateOwnerRefund(ownerId,bookingId,reason){
  ...
  await setBookingStatusById(booking.id,'Refund Pending');await restoreOneBed(booking.roomId);await releaseInventoryLock(booking.id).catch(()=>{});clearCaches();return r;
}
