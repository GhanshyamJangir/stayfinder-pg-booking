...
export const PLUS_SHEETS={
  ...
  StayLifecycle:['booking_id','checkin_otp_hash','checkin_otp_expires','checkin_verified_at','checkout_requested_at','checkout_verified_at','status','updated_at'],
...
export function clearPlusCache(name){if(name)rowCache.delete(name);else rowCache.clear();}
...
export async function ensurePlusSheets(){
  ...
  ensurePromise=(async()=>{
    ...
    if(missing.length){
      await sheets.spreadsheets.batchUpdate({spreadsheetId:sid(),requestBody:{requests:missing.map(title=>({addSheet:{properties:{title}}}))}});
      const data=missing.map(name=>({range:`${name}!A1:${col(PLUS_SHEETS[name].length)}1`,values:[PLUS_SHEETS[name]]}));
...
export async function readRows(name,{fresh=false}={}){
  const cached=rowCache.get(name);
  if(!fresh&&cached&&Date.now()-cached.at<CACHE_TTL)return cached.rows.map(x=>({...x}));
  const headers=PLUS_SHEETS[name];
  if(!headers)throw new Error(`Unknown sheet ${name}`);
  try{
    const sheets=await ensurePlusSheets();
    const r=await sheets.spreadsheets.values.get({spreadsheetId:sid(),range:`${name}!A2:${col(headers.length)}`});
    const rows=(r.data.values||[]).map(row=>Object.fromEntries(headers.map((h,i)=>[h,row[i]??''])));
    rowCache.set(name,{at:Date.now(),rows});
    return rows.map(x=>({...x}));
  }catch(e){
    if(cached)return cached.rows.map(x=>({...x}));
    throw e;
  }
}
...
export async function appendRow(name,obj){
  ...
  const c=rowCache.get(name);if(c)rowCache.set(name,{at:Date.now(),rows:[...c.rows,{...obj}]}); return obj;
}

export async function updateRow(name,id,patch){
  const sheets=await ensurePlusSheets(); const headers=PLUS_SHEETS[name];
  const rows=await readRows(name,{fresh:true}); const idx=rows.findIndex(x=>esc(x[headers[0]])===esc(id));
  if(idx<0)throw new Error(`${name} record not found.`);
  ...
  const c=rowCache.get(name);if(c){const copy=c.rows.map((r,i)=>i===idx?{...next}:{...r});rowCache.set(name,{at:Date.now(),rows:copy});} return next;
}

export async function patchRows(name,predicate,patch){
  const sheets=await ensurePlusSheets(); const headers=PLUS_SHEETS[name];
  const rows=await readRows(name); const data=[]; const out=[];
  rows.forEach((cur,i)=>{if(!predicate(cur))return;const next={...cur,...patch};data.push({range:`${name}!A${i+2}:${col(headers.length)}${i+2}`,values:[[...headers.map(h=>next[h]??'')]]});out.push(next);});
  if(data.length){await sheets.spreadsheets.values.batchUpdate({spreadsheetId:sid(),requestBody:{valueInputOption:'RAW',data}});const c=rowCache.get(name);if(c){const copy=c.rows.map(r=>predicate(r)?{...r,...patch}:{...r});rowCache.set(name,{at:Date.now(),rows:copy});}}
  return out;
}
