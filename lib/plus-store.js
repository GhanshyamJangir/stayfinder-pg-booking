...
export const PLUS_SHEETS={
  ...
  Messages:['id','booking_id','sender_id','sender_role','message','attachment_file_id','attachment_name','attachment_type','delivered_at','read_at','created_at'],
...
export async function ensurePlusSheets(){
  ...
  if(requests.length)await sheets.spreadsheets.batchUpdate({spreadsheetId:sid(),requestBody:{requests}});
...
export async function updateRowsByIds(name,patches=[]){
  if(!patches.length)return [];
  const sheets=await ensurePlusSheets(); const headers=PLUS_SHEETS[name];
  const r=await sheets.spreadsheets.values.get({spreadsheetId:sid(),range:`${name}!A2:${col(headers.length)}`});
  const rows=r.data.values||[]; const data=[]; const out=[];
  for(const item of patches){const idx=rows.findIndex(x=>esc(x[0])===esc(item.id));if(idx<0)continue;const current=Object.fromEntries(headers.map((h,i)=>[h,rows[idx][i]??'']));const next={...current,...item.patch};out.push(next);data.push({range:`${name}!A${idx+2}:${col(headers.length)}${idx+2}`,values:[[...headers.map(h=>next[h]??'')]]});}
