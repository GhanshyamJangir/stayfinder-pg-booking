...
export async function mailAccountCreated(user){return notifyUser(user,{subject:'Welcome to StayFinder',title:`Welcome to StayFinder, ${user.name||user.username}`,lines:[`Your ${user.role==='owner'?'Owner':'Customer'} account has been created successfully.`,`Username: ${user.username}`],buttonText:'Open StayFinder',buttonUrl:appUrl()});}
...
export async function mailBookingCreated(customerId,booking){
  ...
  await notifyUser(guest,{subject:`Booking request submitted • ${full.id}`,title:'Booking request submitted',lines:[...lines,'The Owner will review your request.'],buttonText:'View My Bookings',buttonUrl:`${appUrl()}/customer`});
  await notifyUser(owner,{subject:`New booking request • ${full.id}`,title:'You have a new booking request',lines:[...lines,`Customer: ${guest?.name||'Customer'}`],buttonText:'Review booking',buttonUrl:`${appUrl()}/owner`});
}
...
export async function mailBookingStatus(ownerId,bookingId,status){
  ...
  await notifyUser(guest,{subject:`Booking ${status.toLowerCase()} • ${bookingId}`,title:`Booking ${status}`,lines:[`Booking ID: ${bookingId}`,`Property: ${prop}`,status==='Accepted'?'You can continue to payment from My Bookings.':status==='Rejected'?'The Owner could not accept this request.':'The booking status was updated.'],buttonText:'View My Bookings',buttonUrl:`${appUrl()}/customer`});
  await notifyUser(owner,{subject:`Booking updated • ${bookingId}`,title:`Booking marked ${status}`,lines:[`Booking ID: ${bookingId}`,`Property: ${prop}`,`Customer: ${guest?.name||'Customer'}`],buttonText:'Open Owner dashboard',buttonUrl:`${appUrl()}/owner`});
}
...
export async function mailPaymentSubmitted(customerId,payment){
  ...
  await notifyUser(guest,{subject:`Payment proof submitted • ${payment.bookingId}`,title:'Payment proof submitted',lines:[...lines,'Owner verification is pending.'],buttonText:'View booking',buttonUrl:`${appUrl()}/customer`});
  await notifyUser(owner,{subject:`Payment proof received • ${payment.bookingId}`,title:'Payment verification required',lines:[...lines,`Customer: ${guest?.name||'Customer'}`],buttonText:'Verify payment',buttonUrl:`${appUrl()}/owner`});
}
...
export async function mailPaymentStatus(ownerId,paymentId,status,reason=''){
  ...
  await notifyUser(guest,{subject:`Payment ${status.toLowerCase()} • ${p.bookingId}`,title:`Payment ${status}`,lines:lines.concat(status==='Verified'?['Your stay is confirmed. Owner contact and exact location are now available in My Bookings.']:['Please correct the payment details and submit again.']),buttonText:'View My Bookings',buttonUrl:`${appUrl()}/customer`});
  await notifyUser(owner,{subject:`Payment ${status.toLowerCase()} • ${p.bookingId}`,title:`Payment marked ${status}`,lines,buttonText:'Open Owner dashboard',buttonUrl:`${appUrl()}/owner`});
}
...
export async function mailRefundInitiated(ownerId,refund){
  ...
  await notifyUser(guest,{subject:`Refund initiated • ${refund.bookingId}`,title:'Refund initiated',lines:[...lines,'The Owner will send the refund to your saved refund destination.'],buttonText:'View refund status',buttonUrl:`${appUrl()}/customer`});
...
export async function mailRefundSent(ownerId,refund){
  ...
  await notifyUser(guest,{subject:`Refund sent • ${refund.bookingId}`,title:'Refund sent by Owner',lines:[...lines,'Please confirm after the amount is received.'],buttonText:'Confirm refund',buttonUrl:`${appUrl()}/customer`});
  await notifyUser(owner,{subject:`Refund proof submitted • ${refund.bookingId}`,title:'Refund marked as sent',lines,buttonText:'Open Owner dashboard',buttonUrl:`${appUrl()}/owner`});
}
...
export async function mailRefundCustomerAction(customerId,refundId,action,note=''){
  ...
  await notifyUser(owner,{subject:`${title} • ${r.bookingId}`,title,lines:[...lines,`Customer: ${guest?.name||'Customer'}`],buttonText:'Open Owner dashboard',buttonUrl:`${appUrl()}/owner`});
}
...
export async function mailSupportCreated(user,ticket){
  ...
  const support=process.env.SUPPORT_EMAIL||'stayfinderjaipur@gmail.com';
...
export async function mailReviewSubmitted(customerId,review){
  ...
  await notifyUser(owner,{subject:`New review • ${b.pg?.name||b.pgId}`,title:'Your listing received a review',lines:[`Customer: ${guest?.name||'Customer'}`,`Rating: ${review.rating}/5`,review.comment?`Comment: ${review.comment}`:''].filter(Boolean),buttonText:'Open Owner dashboard',buttonUrl:`${appUrl()}/owner`});
}
